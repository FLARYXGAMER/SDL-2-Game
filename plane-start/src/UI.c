                    ...
                    Uint8 rv, Uint8 gv, Uint8 bv, Uint8 av)
{
    ...
    SDL_RenderDrawLine(r, x,  y,  x+sz, y);   SDL_RenderDrawLine(r, x,  y,  x,  y+sz);
    SDL_RenderDrawLine(r, x2, y,  x2-sz,y);   SDL_RenderDrawLine(r, x2, y,  x2, y+sz);
    SDL_RenderDrawLine(r, x,  y2, x+sz, y2);  SDL_RenderDrawLine(r, x,  y2, x,  y2-sz);
    SDL_RenderDrawLine(r, x2, y2, x2-sz,y2);  SDL_RenderDrawLine(r, x2, y2, x2, y2-sz);
}
...
void renderMenu(SDL_Renderer* renderer, Button buttons[], int count,
                TTF_Font* font, int sizeW, int sizeH, const char* title)
{
    int sw, sh;
    SDL_GetRendererOutputSize(renderer, &sw, &sh);
    SDL_Rect box = {sw/2 - sizeW/2, sh/2 - sizeH/2, sizeW, sizeH};
    int cx = box.x + box.w / 2;

    drawMenuBackground(renderer, sw, sh);
    drawMenuBox(renderer, box);

    // Title
    int titleH = rtextscale(renderer, font, title,
                             (SDL_Color){0, 220, 255, 255}, cx, box.y + 28, 2.2f);

    // Double separator line
    int sepY = box.y + 28 + titleH + 12;
    SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
    SDL_SetRenderDrawColor(renderer, 0, 190, 240, 210);
    SDL_RenderDrawLine(renderer, box.x + 22, sepY,   box.x + box.w - 22, sepY);
    SDL_SetRenderDrawColor(renderer, 0, 80, 120, 100);
    SDL_RenderDrawLine(renderer, box.x + 22, sepY+2, box.x + box.w - 22, sepY+2);

    // Buttons
    layoutButtons(buttons, count, box, sepY + 22 - box.y, 14);
    for (int i = 0; i < count; i++)
